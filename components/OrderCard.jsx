...
const OrderCard = ({ orderInfo }) => {
  ...
  const {addCurrentOrder } = useOrder()
  return (
    <View style={{paddingHorizontal:8, paddingVertical:8, borderColor:'#000000', borderWidth:1, marginHorizontal:8, marginVertical:10, justifyContent:'space-between', flexDirection:'row'} }>
      ...
      <View>
        <Pressable style={{ backgroundColor: '#11ee32', paddingHorizontal: 4, paddingVertical: 8, borderRadius: 6, marginBottom: 5 }} onPress={(event) => {
          addCurrentOrder(orderInfo.orderID)
          navigation.navigate('OrderDetails', { orderID: orderInfo.orderID })
        }}>
